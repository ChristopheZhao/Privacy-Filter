# Structured Extraction Prompt (Priors V4)

Use this prompt for pure-LLM benchmark variants that need the `priors-v3` guardrails plus stricter JSON discipline and better handling of remaining code/config edge cases.
...
- JSON keys and string values must always use double quotes. Never use single quotes as JSON delimiters.
- Escape inner quotes correctly when `context_before` or `context_after` includes quoted code or config text.
...
- Treat real-looking phones, landlines, emails, ID cards, passports, bank cards, WeChat IDs, QQ numbers, secret values, database URLs, IPs, endpoints, names, addresses, and internal system identifiers as redactable when the text context suggests a real person, real system, or real business record.
...
- If an `API_ENDPOINT` uses a literal IP host and explicit port, returning the endpoint plus nested `IP_ADDRESS` and `PORT` is useful and preferred.
...
- Use `ORG_NAME` for team, company, or department names when they identify a real organization in context. Prefer the longest complete organization span, not a shortened suffix like `华北支持组` when the full text says `腾讯会议华北支持组`.
...
- Landline formats such as `0755-22334455` or similar area-code numbers should be labeled `PHONE_NUMBER`.
...
- Positive example: `spring.datasource.url=jdbc:mysql://dbadmin:pwd123@172.18.2.9:3306/reporting\nservice.endpoint=https://10.18.9.5:8443/v2` -> return `DATABASE_URL` for the JDBC string and `API_ENDPOINT` / `IP_ADDRESS` / `PORT` for the HTTPS endpoint, never `DATABASE_URL` for the HTTPS span.
...
- Positive example: `联系座机：0755-22334455，QQ号：65432109，API_KEY="sk-edge-77889900"` -> return `PHONE_NUMBER`, `QQ_NUMBER`, and `SENSITIVE_VALUE` for `sk-edge-77889900`.
